...
pub struct Feed {
    ...
    pub url: String,
    pub title: String,
    /// User-supplied folder label.  None until the user explicitly sets one.
    /// Diatom never sets this automatically.
...
impl RssStore {
    ...
    pub fn add(
        ...
        let raw = crate::storage::db::RssFeedRaw {
            id: id.clone(),
            url: url.to_owned(),
            title: url.to_owned(),
            category,
    ...
    pub fn remove_feed(&mut self, id: &str, db: &crate::storage::db::Db) {
    ...
    pub fn ingest(&mut self, feed_id: &str, xml: &str, db: &crate::storage::db::Db) -> u32 {
        let mut count = 0u32;
        let now = crate::storage::db::unix_now();
    ...
    pub fn mark_read(&mut self, item_id: &str, db: &crate::storage::db::Db) {
...
fn feed_to_raw(f: &Feed) -> crate::storage::db::RssFeedRaw {
    crate::storage::db::RssFeedRaw {
        id: f.id.clone(),
        url: f.url.clone(),
        title: f.title.clone(),
